...
public class LunarCalendarUtil {
    ...
    private static LunarCalendarUtil instance;
    private LunarCalendarUtil (){}
    public static synchronized LunarCalendarUtil getInstance() {
        if (instance == null) {
            instance = new LunarCalendarUtil();
        }
        return instance;
    }
    ...
    public static String get(Resources resources, Calendar calendar) {
        // 农历时间显示
        ...
        int year = lunar[0], month = lunar[1] - 1, day = lunar[2];

        String monthString, dayString;

        ArrayList<String> lunarMonth = ChinaDate.getMonths(resources, year);

        int leapMonth = ChinaDate.leapMonth(year);
        if (leapMonth != 0 && (month > leapMonth - 1 || isLeap)) { //选中月是闰月或大于闰月
            monthString = lunarMonth.get(month + 1);

        } else {
            monthString = lunarMonth.get(month);
        }

        if (ChinaDate.leapMonth(year) == 0) { // 判断大小月及是否闰年,用来确定"日"的数据
            dayString = ChinaDate.getLunarDays(ChinaDate.monthDays(year, month)).get(day - 1);
        } else {
            dayString = ChinaDate.getLunarDays(ChinaDate.leapDays(year)).get(day - 1);
        }

        return resources.getString(R.string._xpopup_ext_lunar_calendar) + monthString + dayString;
    }
    // ====== 传回农历 y年闰月的天数
