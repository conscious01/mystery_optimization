...
import com.zgzx.metaphysics.city_time_picker.xpopupext.adapter.NumericWheelAdapter;
import com.zgzx.metaphysics.city_time_picker.xpopupext.listener.ISelectTimeCallback;
import com.zgzx.metaphysics.city_time_picker.xpopupext.utils.ChinaDate;
import com.zgzx.metaphysics.city_time_picker.xpopupext.utils.LunarCalendar;
...
public class WheelTime {
    ...
    private static final int DEFAULT_START_YEAR = 1930;
    ...
    private void setLunar(int year, final int month, int day, boolean isLeap, int h, int m, int s) {
        ...
        wv_month.setAdapter(new ArrayWheelAdapter(ChinaDate.getMonths(resources, year)));
        wv_month.setLabel("");
        ...
        if (ChinaDate.leapMonth(year) == 0) { // 判断大小月及是否闰年,用来确定"日"的数据
        ...
        wv_hours.setAdapter(new ArrayWheelAdapter(ChinaDate.getLunarHours(resources)));
        ...
        wv_year.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                // 判断是不是闰年,来确定月和日的选择
                wv_month.setAdapter(new ArrayWheelAdapter(ChinaDate.getMonths(resources, year_num)));
    ...
    private void setSolar(int year, final int month, int day, int h, int m, int s) {
        ...
        wv_hours.setAdapter(new ArrayWheelAdapter(ChinaDate.getHours(resources)));
...
//        }
    ...
    public String getString() {
        ...
        String lunar_calendar = resources.getString(R.string._xpopup_ext_lunar_calendar);
        String gregorian_calendar = resources.getString(R.string._xpopup_ext_gregorian_calendar);

        if (isLunarCalendar) {
            return lunar_calendar + " "
                    + wv_year.getAdapter().getItem(wv_year.getCurrentItem())
                    + " " + wv_month.getAdapter().getItem(wv_month.getCurrentItem())
                    + " " + wv_day.getAdapter().getItem(wv_day.getCurrentItem())
                    + " " + wv_hours.getAdapter().getItem(wv_hours.getCurrentItem());
        }

        Object year = wv_year.getAdapter().getItem(wv_year.getCurrentItem()),
                month = wv_month.getAdapter().getItem(wv_month.getCurrentItem()),
                day = wv_day.getAdapter().getItem(wv_day.getCurrentItem());
        //hour = wv_hours.getAdapter().getItem(wv_hours.getCurrentItem());

        return gregorian_calendar + " "
                + year + view.getContext().getString(R.string._xpopup_ext_year)
                + " " + month + view.getContext().getString(R.string._xpopup_ext_month)
                + " " + day + view.getContext().getString(R.string._xpopup_ext_day)
                + "  " + wv_hours.getAdapter().getItem(wv_hours.getCurrentItem());
    }
    ...
    private String getLunarTime() {
        StringBuilder sb = new StringBuilder();
        int month; // 月
        int year = wv_year.getCurrentItem() + startYear; // 年
        ...
        // 未知时辰
        if (wv_hours.getCurrentItem() == 0) {
            return sb.append(solar[0]).append("-")
                    .append(solar[1]).append("-")
                    .append(solar[2]).append(" 00:00:00")
                    .toString();
        }
        ...
        Log.i(WheelTime.class.getSimpleName(), String.valueOf(wv_hours.getCurrentItem()));
        return sb.toString();
    }
